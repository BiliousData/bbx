...
{
	//Check which icon type gets used
	s8 dying;
	if (ox < 0)
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	//Load HUD textures
	//Yandere simulator code hell yeaaaah
	if (stage.stage_id == StageId_1_1)
	    ...
	    Gfx_LoadTex(&stage.tex_hud0p, IO_Read("\\STAGE\\HUD0BF.TIM;1"), GFX_LOADTEX_FREE), Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0SIP.TIM;1"), GFX_LOADTEX_FREE);
	...
	if (stage.stage_id == StageId_1_2 && stage.song_step == 1) //Vomit
		Gfx_LoadTex(&stage.tex_hud0e, IO_Read("\\STAGE\\HUD0SIP.TIM;1"), GFX_LOADTEX_FREE);
